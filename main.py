...
async def on_message(message):
    ...
    if message.content.startswith("}help"):
      if coinflip() == 1:
        ...
        await message.channel.send("""
          :hot_face: **Slave's Commands** :hot_face::
    ...
    # if message.author.name == "pleasestepbro":
    #   await message.channel.send('Anh is hot')
    ...
    if message.content.startswith("}minecraft"):
      ...
      encounters = [blacksquare,pig,tree,rock]

      playField = [[blacksquare for x in range(11)] for x in range(2)] + [[greensquare for x in range(11)]]
      playField[1][10] = man
      ...
      while(len(health) > 1):
        ...
        if reaction.emoji == "🔪":
          inventory["pork"] += random.randint(1,3)
